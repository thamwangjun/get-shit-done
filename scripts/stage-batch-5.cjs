...
function run() {
  ...
  // Check if "chore(maintenance): refactor quick tasks, logs, and state files (Batch 5)" exists anywhere in git history.
  // If so, print "Batch 5 already committed" and exit 0.
  let batch5InHistory = false;
  try {
    const allCommits = execFileSync('git', ['log', '--pretty=format:%s'], { encoding: 'utf8', cwd: repoRoot });
    batch5InHistory = allCommits.includes('chore(maintenance): refactor quick tasks, logs, and state files (Batch 5)');
  } catch (err) {
    console.warn('Warning: Could not check git history:', err.message);
  }

  if (batch5InHistory) {
    console.log('Batch 5 already committed');
    process.exit(0);
  }

  // Initialize a Set of expected files (hardcoded for Batch 5).
  ...
  // Handle the logs/ untracked directory: stage all files in it and expand expectedFiles.
  const logsDir = path.join(repoRoot, 'logs');
  if (fs.existsSync(logsDir)) {
    console.log('Staging logs/ directory...');
    try {
      execFileSync('git', ['add', '-f', 'logs/'], { cwd: repoRoot });
    } catch (err) {
      console.error('Failed to git add logs/:', err.message);
      process.exit(1);
    }
    const logFiles = fs.readdirSync(logsDir);
    for (const f of logFiles) {
      expectedFiles.add(`logs/${f}`);
    }
  }
  ...
  for (const file of expectedFiles) {
    if (hasChangesSinceV1_41_2(file, repoRoot)) {
      console.log(`Staging modified Batch 5 file: ${file}`);
      try {
  ...
  // If there are no staged files at all, log "No modifications to stage for Batch 5" and exit 0.
  if (stagedFiles.length === 0) {
    console.log('No modifications to stage for Batch 5');
    process.exit(0);
  }

  // If validation passes and files are staged, execute the Batch 5 commit.
  console.log('Committing staged Batch 5 files...');
  try {
    execFileSync('git', ['commit', '-m', 'chore(maintenance): refactor quick tasks, logs, and state files (Batch 5)'], { cwd: repoRoot, stdio: 'inherit' });
    console.log('Batch 5 commit successful.');
  } catch (err) {
    console.error('Failed to commit Batch 5 changes:', err.message);
    process.exit(1);
  }
}
