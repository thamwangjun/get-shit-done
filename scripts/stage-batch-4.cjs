...
function run() {
  ...
  // Check if "test: refactor core tests and SDK validation (Batch 4)" exists anywhere in git history.
  // If so, print "Batch 4 already committed" and exit 0.
  let batch4InHistory = false;
  try {
    const allCommits = execFileSync('git', ['log', '--pretty=format:%s'], { encoding: 'utf8', cwd: repoRoot });
    batch4InHistory = allCommits.includes('test: refactor core tests and SDK validation (Batch 4)');
  } catch (err) {
  ...
  // Dynamically scan tests/ for .test.cjs files.
  ...
  // If validation passes and files are staged, execute git commit -m "test: refactor core tests and SDK validation (Batch 4)"
