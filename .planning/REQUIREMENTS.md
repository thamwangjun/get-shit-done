...
- [x] **INTG-01**: `eta` v4 added to `dependencies` in `package.json`; a module-level Eta instance configured with default `<%`/`%>` delimiters (no `tags:` or `parse.raw:` overrides), `autoEscape: false`, `useWith: true`, and `views` = repo root exists in `bin/install.js`
