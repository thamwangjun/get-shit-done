...
function resolveReasoningEffortInternal(cwd, agentType) {
  ...
  const phaseTypeTierRaw = (phaseType && config.models && typeof config.models === 'object')
    ? config.models[phaseType]
    : undefined;
  const phaseTypeTierBase = phaseTypeTierRaw ? parseModelEffort(phaseTypeTierRaw).model : undefined;
  const VALID_TIERS = new Set(['opus', 'sonnet', 'haiku', 'inherit']);
  const fromPhaseType = !!(phaseTypeTierRaw && VALID_TIERS.has(phaseTypeTierBase));
